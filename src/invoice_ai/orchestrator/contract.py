...
def conversation_state_for(*, request_kind: str, response: ToolResponse) -> dict[str, Any]:
    ...
    if request_kind in {"invoice_draft", "invoice_revision"}:
        active_invoice = {
        ...
        if isinstance(revision, dict):
            active_invoice["latest_revision_id"] = revision.get("revision_id")
            active_invoice["preview_path"] = revision.get("preview_path")

        preview = data.get("preview")
        if isinstance(preview, dict) and preview.get("path"):
            active_invoice["preview_path"] = preview["path"]

        return {"active_invoice": active_invoice}
...
def next_request_contract(*, request_kind: str, response: ToolResponse) -> dict[str, Any]:
    ...
    if request_kind == "invoice_draft":
        ...
        return {
            ...
            "planner_transition": (
                "A later chat-planner may translate free-form operator turns into this "
                "structured invoice_revision envelope while reusing conversation_context.active_invoice."
            ),
            "supported_follow_up": {
                ...
                "payload_shape": {
                    ...
                    "summary": "Describe the sales invoice revision being applied",
                },
            },
        }
    ...
    if request_kind == "invoice_revision":
        return {
            "contract_version": "structured-v1",
            "planner_transition": (
                "Keep routing follow-up sales invoice edits through invoice_revision while the planner "
                "layer remains responsible only for turning chat into safe patch payloads."
            ),
        }
